...
import { Lock, LogOut, User, Crown, Zap, Sparkles, Settings } from "lucide-react"
...
export default function MembrosPage() {
  ...
  const checkAccess = async () => {
    try {
      ...
      const subStatus = await checkSubscriptionStatus(currentUser.id, currentUser.email)
  ...
  const getStatusBadge = (status: string, isVip: boolean) => {
    if (isVip) {
      return <Badge className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white border-0">VIP</Badge>
    }
    ...
    switch (status) {
      ...
      case 'trialing':
        return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">Período de teste</Badge>
      default:
  ...
  const PlanIcon = profile?.plan_type ? getPlanIcon(profile.plan_type) : Lock
    ...
    <div className="min-h-screen bg-[#0B0B0D]">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Link href="/perfil">
                <Button variant="ghost" className="text-[#CFC8C4] hover:text-[#F7F3F1] hover:bg-[#F7F3F1]/5">
      ...
      <div className="container mx-auto px-4 py-12">
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-3xl text-[#F7F3F1] mb-2">
                  Bem-vindo(a), {profile?.full_name || user?.email?.split('@')[0]}! 👋
                </CardTitle>
              ...
              <div className="text-right">
                {getStatusBadge(subscriptionInfo?.status, subscriptionInfo?.isVip)}
                {profile?.plan_type && (
                  <div className="flex items-center gap-2 mt-2 text-[#CFC8C4]">
                    <PlanIcon className="w-4 h-4" />
                    <span className="text-sm">Plano {getPlanName(profile.plan_type)}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Exemplo de cards de conteúdo */}
          {[1, 2, 3, 4, 5, 6].map((item) => (
            <Card key={item} className="bg-[#121214] border-[#9A8F88]/20 hover:border-[#C69A5B]/50 transition-all cursor-pointer group">
              <CardHeader>
                ...
                <CardTitle className="text-[#F7F3F1]">Conteúdo Exclusivo #{item}</CardTitle>
